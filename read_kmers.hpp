...
int kmer_size(const std::string &fname) {
	...
	if (!fin.is_open()) { throw std::runtime_error("kmer_size: could not open " + fname); }
...
size_t line_count(const std::string &fname) {
	...
	if (f == NULL) { throw std::runtime_error("line_count: could not open " + fname); }
	size_t n_lines = 0;
	int n_read;
...
std::vector<kmer_pair> read_kmers(const std::string &fname, int nprocs = 1, int rank = 0) {
	...
	FILE *f = fopen(fname.c_str(), "r");
	if (f == NULL) { throw std::runtime_error("read_kmers: could not open " + fname); }
...
std::string extract_contig(const std::list<kmer_pair> &contig) {
	...
	for (const auto &kmer : contig) {
		if (kmer.forwardExt() != 'F') { contig_buf += kmer.forwardExt(); }
	}
	return contig_buf;
}
